...
from app.utils.config import RUNNER_URL

class RunnerError(RuntimeError): ...

def ensure_runner_session(client: str, flow_ref: str) -> str:
    r = requests.post(f"{RUNNER_URL}/runner/session", json={"client": client, "flow_ref": flow_ref}, timeout=10)
...
def run_step(session_id: str, agent_ref: str, flow_ref: str, step: Dict[str,Any]) -> Dict[str,Any]:
    ...
    r = requests.post(f"{RUNNER_URL}/runner/step", json=payload, timeout=120)

...
3. **Wake-up & règles (Core Archiviste)** : `.openAi-provider/.codex-Archiviste/WAKEUP-LINK.md` — Retrouve intents, guardrails, références messaging.
4. **Contexte session** : Priorités, ordres actifs, backlog Core Guardians.

## Actions immédiates avant toute réponse

- **Langue** : parle, raisonne et rédige uniquement en français.
- **Messagerie** : exécute `npm run arkamsg -- pull --agent arka-agent00-core-archivist` dès l'ouverture de session, vérifie si des messages sont présents puis applique la procédure ARKORE20.
